...
function HistoryPage() {
  ...
  return (
    ...
    >
      ...
      {openLog && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in" onClick={() => setOpenLog(null)}>
          <div className="max-w-2xl w-full max-h-[85vh] overflow-y-auto rounded-3xl bg-gradient-to-br from-slate-900 to-slate-800 border-2 border-white/20 shadow-2xl p-5 text-white animate-in zoom-in" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-xl font-extrabold">📜 Log da batalha</h2>
              <button onClick={() => setOpenLog(null)} className="text-white/70 hover:text-white text-2xl leading-none">×</button>
            </div>
            <div className="text-xs opacity-70 mb-3">{new Date(openLog.created_at).toLocaleString("pt-BR")}</div>
            <div className="space-y-1 text-sm font-mono bg-black/40 rounded-xl p-3 border border-white/10">
              {Array.isArray(openLog.log) && openLog.log.length > 0 ? (
                (openLog.log as unknown[]).map((line, i) => (
