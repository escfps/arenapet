...
-- Add Fenix Vermelha e Fenix Negra ao roster de bots
...
BEGIN
  pool := CASE rarity
    ...
    WHEN 'mythic' THEN ARRAY['dragao_branco','dragao_negro','fenix_vermelha','fenix_negra']
...
RETURNS text LANGUAGE sql IMMUTABLE SET search_path TO 'public' AS $function$
  SELECT CASE
    ...
    WHEN sp IN ('dragao_branco','dragao_negro','fenix_vermelha','fenix_negra') THEN 'mythic'
    ELSE 'common' END
$function$;
...
RETURNS text LANGUAGE sql IMMUTABLE SET search_path TO 'public' AS $function$
  SELECT CASE sp
    ...
    WHEN 'jacare_ancestral' THEN 'Jacaré Ancestral' WHEN 'gorila_titan' THEN 'Gorila Titan'
...
LANGUAGE sql IMMUTABLE SET search_path TO 'public' AS $function$
  SELECT t.hp, t.atk, t.def, t.spd FROM (VALUES
    ...
    ('fenix_vermelha',95,30,16,18),('fenix_negra',100,28,17,17)
